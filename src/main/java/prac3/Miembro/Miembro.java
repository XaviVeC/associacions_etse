...
public abstract class Miembro {
    ...
    public Fecha fechaMasAnteriorDeMiembro(ListaAsociaciones listaTodasAsociaciones) {
        ...
        for (int indiceAsociaciones = 0; indiceAsociaciones < listaTodasAsociaciones
            ...
            hecho = false;
            while ((indiceListaMiembrosAsociacion < listaTodasAsociaciones
                    .getElementoListaAsociacion(indiceAsociaciones).getListaMiembrosAsociacion().length) && (!hecho)) {
                if (listaTodasAsociaciones.getElementoListaAsociacion(indiceAsociaciones)
                        .getListaMiembrosAsociacion()[indiceListaMiembrosAsociacion].equals(alias)) {
                    if (listaTodasAsociaciones.getElementoListaAsociacion(indiceAsociaciones)
                            .getFechasBaja()[indiceListaMiembrosAsociacion].getDia() == 99) {
                        if (fechaAnterior == null) {
                            fechaAnterior = listaTodasAsociaciones.getElementoListaAsociacion(indiceAsociaciones)
                                    .getFechasAlta()[indiceListaMiembrosAsociacion].copia();
                            hecho = true;
                        } else {
                            if (fechaAnterior.compararFechas(
                                    listaTodasAsociaciones.getElementoListaAsociacion(indiceAsociaciones)
                                            .getFechasAlta()[indiceListaMiembrosAsociacion]) == 2) {
                                fechaAnterior = listaTodasAsociaciones.getElementoListaAsociacion(indiceAsociaciones)
                                        .getFechasAlta()[indiceListaMiembrosAsociacion].copia();
                            } else {
                                if (fechaAnterior.compararFechas(
